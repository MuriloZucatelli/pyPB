...
ct_data = dict(
    [
        (c, genfromtxt("./pbe/app/data/coulaloglou/d32_N_alpha{0}.txt".format(c)))
...
for c in concentrations:
    ax.plot(
        ...
        marker=next(marker),
        linestyle="",
        label=r"C\&T $\phi={0:0.2f}$".format(c / 100.0),
    )
...
for c in concentrations:
    ax.plot(
        ...
        label=r"Num. $\phi={0:0.2f}$".format(c / 100.0),
        marker="d",
    )
    # plt.plot(
...
first_legend = ax.legend(
    handles[len(concentrations) :], labels[len(concentrations) :], loc="upper right"
)
ax.add_artist(first_legend)
second_legend = ax.legend(
    handles[: len(concentrations)], labels[: len(concentrations)], loc="lower left"
)
